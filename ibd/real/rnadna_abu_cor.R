...
names(cor) <- names(sdf)
cor <- unlist(cor)
saveRDS(cor,file='ibd/real/plot/cor/rnadna_abu_cor.rds')

library(ggplot2)
pdf('ibd/real/plot/cor/rnadna_abu_hist.pdf',width=3,height=3)
...
pn <- names(sort(cor[highn],decreasing = T)[3])
ss <- sdf[[pn]]
r1 <- lm(rna~abu,data=ss)
r2 <- lm(dna~abu,data=ss)

pdf(paste0('ibd/real/plot/cor/rnadna_abu_',pn,'.pdf'),width=3,height=3)
ggplot(data.frame(x=resid(r1),y=resid(r2)),aes(x=x,y=y)) + geom_point(color='orange') + geom_smooth(method='lm',color='royalblue',se=F) + theme_classic() + xlab('RNA | Taxa') + ylab('DNA | Taxa') + theme(legend.position = 'none') + ggtitle(paste0('Correlation:',round(cor(resid(r1),resid(r2)),3)))
dev.off()

pn <- names(sort(cor[highn],decreasing = T)[150])
ss <- sdf[[pn]]
r1 <- lm(rna~abu,data=ss)
r2 <- lm(dna~abu,data=ss)
...
pn <- names(sort(cor[highn],decreasing = F)[10])
ss <- sdf[[pn]]
r1 <- lm(rna~abu,data=ss)
r2 <- lm(dna~abu,data=ss)
